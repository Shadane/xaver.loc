...
{//Запускается циклом на 89 строке.  эта функция работает со скидками и в зависимости от значений отправляет данные в функцию diskont
    global $bd;
    ...
    } else
    {
        ...
        {
            case 'diskont1':
                bd_push('Скидки', 'Ваша скидка на "'.$key.'" составляет: ', ' 10%');
                ...
                break;
            case 'diskont2':
                bd_push('Скидки', 'Ваша скидка на "'.$key.'" составляет: ', ' 20%');
                $diskont_var_func = 'diskont';
                $diskont_var_func($key, '20');
                break;

            default:
                bd_push('Скидки', 'Ваша скидка на "'.$key.'" составляет: ', ' 0%');
                $diskont_var_func = 'diskont';
...
if (!function_exists('diskont')) 
{ //эта функция ведет почти все рассчеты.Кстати не знаю стоит ли каждый раз писать вот так if !function exists или не нужно.

    function diskont($bd_key = '', $discounter_percent = '0', $return = '') 
    {  //ключ и проценты получает из предыдущей функции, 
        ...
        if (!$return) 
        {                                                     //если третьего параметра не задано, то 
...
{ //цикл, который запускает обработку скидок для каждого наименования
    if ($bd[$key]['количество заказано']!=0)
        { //на случай, если появится кнопочка '-' у каждого товара в корзине)
...
//цикл, который выводит массив на экран. В этом месте заметил сложность в случае, если не нужно выводить какую-то позицию. Например если их будет заказано 0.
//Ведь если до заполнения массива сравнение по типу $bd[$key]['количество заказано']== прокатывало, то при добавлении в него Итого, Скидки, Уведомлений это уже не сработает.
//Так что если нужно что-то удалить из корзины, то в текущем варианте получится внушительная конструкция. 
//Возможное решение - запись через bd_push в другой массив, вывод $bd через цикл на строке 87, а циклом ниже выводить второй массив. Переделаю если понадобится.
...
{ 
    ...
    foreach ($massiv as $inner_key => $value) 
    {
        ...
        elseif ($inner_key !== 'diskont') 
            
        {//чтобы при выводе не показывалась графа diskont
